...
class ScoreParams:
    ...
    def mis_match_char(self, x: int, y: int) -> int:
        if x != y:
...
def global_align(
    x: List[int], y: List[int], score: ScoreParams
...
def get_aligned_sequences(
    x: List[int], y: List[int], trace_back: np.ndarray
) -> Tuple[List[Union[str, int]], List[Union[str, int]], torch.Tensor]:
    x_seq: List[Union[str, int]] = []
    y_seq: List[Union[str, int]] = []
    i = len(x)
    j = len(y)
    mapper_y_to_x: List[Tuple[int, int]] = []
    while i > 0 or j > 0:
        if trace_back[i, j] == 3:
            x_seq.append(x[i - 1])
            y_seq.append(y[j - 1])
...
def get_mapper(
    x: str, y: str, tokenizer: CLIPTokenizer, max_len: int = 77
...
def get_refinement_mapper(
    prompts: List[str], tokenizer: CLIPTokenizer, max_len: int = 77
) -> Tuple[torch.Tensor, torch.Tensor]:
    x_seq = prompts[0]
    mappers, alphas = [], []
